...
2. They make the two identifiers discoverable from the IG's HTML browse, with description + author + license + status — without embedding any CQL (the CQL coordinate convention treats `urn:cql:library:` as the authoritative identifier across engines).
3. They are documented as *external pointers*: no CQL is embedded in, executed by, or distributed with this IG; the Library resource exists for resolvability and conformance, not for distribution.
