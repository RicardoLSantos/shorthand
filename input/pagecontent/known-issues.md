...
## Build status — v0.5.2 (2026-09-23)

| Metric | Value |
|---|---|
| Errors | **0** (zero since v0.4.1, 2026-06-01) |
| Warnings | **226** — a single advisory class, see below |
| Information | 13,278 |
| Broken links | 0 |
| Toolchain | IG Publisher 2.3.4 · SUSHI 3.20.1 · FHIR 4.0.1 (the same IG Publisher version as the CI) |

## The 226 warnings: one advisory class, not suppressed

Every one of the 226 warnings is the same message: *"The resource … should have an OID assigned to cater for possible use with OID based terminology systems"* — one per CodeSystem (19) and one per ValueSet (207), which is why the count equals the number of terminology resources. OIDs are optional identifiers in FHIR; assigning 226 of them requires an OID arc registered to the publisher, which is a governance decision outside a release. The warning is therefore left visible rather than suppressed, and the count is expected to move only when terminology resources are added or removed.

## Suppressed warnings (`input/ignoreWarnings.txt`, 170 non-comment lines — the first is the file header, so 169 rules: 98 written against the messages of the local builds (twelve of them already in English) and 71 in the block of English twins for the CI runner)
...
| CQL expression language | `text/cql-identifier` not supported for validation | valid CQL media type; the `Library` resources are documentation pointers and no CQL is distributed (see *Open limitations*) |
...
The release builds are made locally with the IG Publisher version recorded in the changelog (2.2.10 for 0.5.0 and 0.5.1, 2.3.4 for 0.5.2); the CI workflow downloads the latest IG Publisher release at build time, so it can run a newer validator than the release did. Measured on 2026-09-17: the CI build of v0.5.0 (and of the commits that followed it) ran IG Publisher **2.3.4** and **failed** its error gate with 4 errors, all `MEASURE_M_CRITERIA_CQL_NO_LIB` on the `MindfulnessProgressReport` Measure — its four criteria were declared as `text/cql-identifier` without a CQL Library, which 2.2.10 only warned about. 0.5.1 corrects the Measure (the criteria are FHIRPath and are now declared as such; the weekly stratifier expression, which was not valid FHIRPath, is now the observation's effective date) — reproduced with the standalone FHIR validator 6.10.4, the same core as 2.3.4: 4 errors → 0. The 2.3.4 run also reported 361 warnings instead of 223: the runner emits its messages in English and the suppressions were written in the locale of the local builds, so 0.5.1 adds the English twin of every suppression the runner reported (68 lines, full text copied from the run). Every one of the 138 non-OID warnings of that run has a suppression in the local locale, so each now has its English twin (68 lines, written as the whole message: the publisher matches a suppression only against the full text); the OID advisories stay visible by design. Measured on 2026-09-21 on the v0.5.1 commit (run 35599489489, IG Publisher 2.3.4): **0 errors, 229 warnings, 0 broken links** — the 225 OID advisories plus 4 `MEASURE_M_CRITERIA_UNKNOWN` warnings, the English form of the text/fhirpath message, which the 2.3.4 run of v0.5.0 had reported as an error and therefore had no English twin; that twin was added on `main` right after the release (one line, 169 rules) and the next run (35602187134, IG Publisher 2.3.4) showed **0 errors, 225 warnings — the OID advisories only — and 0 broken links**. For 0.5.2 the local publisher was upgraded to 2.3.4, the version the CI runs: the release build of 2026-09-23 gave **0 errors, 226 warnings — the OID advisories only — and 0 broken links**, the same warning count as the CI run on the preceding commit (run 35712764595, 2026-09-22).
...
- **No CQL is distributed or executed.** `ClinicalImpression.protocol` references `urn:cql:library:` URIs registered as thin `Library` resources (documentation pointers); no CQL engine runs in this project. Likewise the GDL2 bridge is documented without an engine.
...
- **Three ConceptMap elements are declared unmatched pending a replacement** (two in the nutrition-to-LOINC map, one in the social-history-to-LOINC map); each carries an inline note rather than a silent approximation. The two `ReproductiveObservation` components and the vendor distance element that 0.5.0 marked for review were decided in 0.5.1 (see the change log); no binding is marked for review in the FSH sources of this release. See the [terminology verification page](terminology-verification.html).
- **Narrative pages carried codes and one figure that a review found wrong** (at least 18 codes quoted only in page text, and an "86 % terminology gap" figure repeated five times on four pages); they are listed in the 0.5.1 change log. The source pages were corrected in 0.5.2 (see *Fixed* in the 0.5.2 change log); the v0.5.1 package still renders the old text. No resource of the release and no ledger row is affected.
