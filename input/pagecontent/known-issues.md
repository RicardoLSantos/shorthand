...
- **Narrative pages carried codes and one figure that a review found wrong** (at least 18 codes quoted only in page text, and an "86 % terminology gap" figure repeated five times on four pages); they are listed in the 0.5.1 change log. The source pages are corrected (see *Fixed* under *Unreleased* in the change log) and the correction ships with 0.5.2; the v0.5.1 package still renders the old text. No resource of the release and no ledger row is affected.
