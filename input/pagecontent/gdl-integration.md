...
- The **CQL engine** is external (the `Library` resources [`LibraryCVR003HRVRisk`](Library-LibraryCVR003HRVRisk.html) and [`LibraryMET002MetabolicOverride`](Library-LibraryMET002MetabolicOverride.html) are doc-pointers via `urn:cql:library:` with `special-url`; no CQL is embedded in, executed by, or distributed with this IG).
...
- The **OMOP ETL pipeline** is external (see [OMOP Integration](omop-integration.html); the IG ships six ConceptMaps but no transformation engine; the HEADS-ETL pipeline, today a proof of concept on synthetic bundles, is positioned to conform to the Vulcan FHIR-to-OMOP IG v1.0.0 rather than as a novel framework — anti-overclaim per Pitfall #110).
