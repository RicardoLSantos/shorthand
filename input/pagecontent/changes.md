...
- **Every concrete profile has a standalone example** — the v0.5.0 build listed two profiles without one: `ConsumerECGObservation`, which is abstract, and `CarePlanLifestyleMedicine`, which had only inline instances inside the AI workflow bundles; a standalone care-plan example is added, so the 0.5.1 build should list 102 of 102 concrete profiles with at least one example (the abstract base profile excluded) — the figure is read from the build, not asserted here.
