...
## Version 0.5.0 (candidate, 2026-09-16)
...
- **openEHR mappings on the profiles that have a genuine archetype** — five `Mapping:` blocks (`input/fsh/mappings/OpenEHRMappings.fsh`, rendered on each profile's *Mappings* tab): `HeartRateVariabilityObservation` → `openEHR-EHR-OBSERVATION.heart_rate_variability.v0`, `VO2MaxEstimationObservation` → `openEHR-EHR-OBSERVATION.vo2max_estimation.v0`, `SleepObservation` → `openEHR-EHR-OBSERVATION.sleep_architecture.v0`, `WearableSensorDevice` and `WearableDataSource` → `openEHR-EHR-CLUSTER.wearable_device.v0`. Every target path names a node present in the `definition` section of the ADL 1.4 archetype and is written with its full cluster containment (for example `items[at0009]/items[at0010]/value`); where the archetype's metric differs from the profile's, the mapping comment says so.
