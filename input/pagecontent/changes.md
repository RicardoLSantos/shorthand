...
- **Bindings under review, marked inline** — each line carries a dated note and the code stays until it is replaced: fibre, saturated-fat and caffeine intake 24 hour (LOINC offers only *Estimated* and *Measured* variants), *Eating habits* (65968-0), household members (63503-7), *Diet* (75282-4), *Fair* bound to SNOMED CT 260347006, whose meaning is the grade *+* (a *Fair* qualifier exists only in the US extension, 445511000124105), in two value sets, average glucose in `ConceptMapCGMToOMOP` (41653-7), distance → steps in `ConceptMapVendorToLOINC`, and the `ReproductiveObservation` frequency (92656-8, a quantitative concept bound to a coded value) and pattern (64699-2, a PhenX questionnaire item with status TRIAL) components.
- Terminology pages (ICD-11 Integration, Design Decisions, Known Issues, Terminology Verification, openEHR Integration, Getting Started) updated accordingly; the Terminology Verification page's ledger section re-generated (436 codes; findings dated 2026-09-15 and 2026-09-16).
