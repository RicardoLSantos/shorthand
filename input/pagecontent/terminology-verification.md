...
Every externally-defined code bound in the FSH sources is listed in `input/data/terminology-verification-ledger.csv` with the date, source and version of its last verification; units written as `system`/`code` pairs on Quantity values (about fifty UCUM codes) are validated by the build but are not yet extracted into the ledger. The ledger is produced by `.github/scripts/terminology_ledger_check.py` (run by hand and weekly as an advisory step of the ConceptMap Drift workflow) and never sits on the build path: the IG validates without a terminology server, and network failures are recorded as "unreachable", never as "not found".
...
|---|--:|---|---|---|--:|
| LOINC | 207 | 2026-09-16 | Athena OMOP snapshot, tx.fhir.org | Athena LOINC snapshot 2026-01-21; tx LOINC 2.82 | 11 |
| SNOMED CT (International) | 169 | 2026-09-15 | Vocab2 OMOP snapshot, tx.fhir.org | Vocab2 SNOMED 2025-02-01 SNOMED CT International Edition; tx SNOMED http://snomed.info/sct/900000000000207008/version/20250201 | 10 |
...
| UCUM | 9 | 2026-09-15 | Athena OMOP snapshot, tx.fhir.org, ucum.nlm.nih.gov validator | Athena UCUM snapshot 2026-01-21; tx UCUM 2.2 | 9 |
...
**Findings on 2026-09-16.** The four rows that carried that note — the `ReproductiveObservation` component codes 72514-3, 3144-3, 92656-8 and 64699-2 — were confronted with the official names in the Athena snapshot and on tx.fhir.org (LOINC 2.82) and now carry them; no row remains with the note. Two of the four bindings are marked in place for review: 92656-8 (*Number of menstrual periods per year*, a quantitative concept — scale Qn, property NRat) is bound to a coded frequency value, and 64699-2 (a PhenX questionnaire item on the pattern of menstrual cycles at ages 18–22, status TRIAL in LOINC 2.82) is bound to a symptom-progression value set. Of the nine bindings that still named another concept on 2026-09-15, eight were resolved on 2026-09-16: fibre, saturated-fat and caffeine intake 24 hour now use the *Estimated* variants 81133-1, 81033-3 and 80489-8 (the code this IG already used for caffeine); 65968-0, 63503-7, 75282-4 and 260347006 were retired from their value sets, the ConceptMap elements that pointed to them declared *unmatched*, and the `ConceptMapCGMToOMOP` element that carried 41653-7 under the name of mean glucose was removed; the ledger holds 431 codes (207 LOINC, 169 SNOMED CT, 46 ICD-11, 9 UCUM) with 30 open findings, all display-name classes. One of the nine remains marked in place — the vendor distance → steps element — alongside the two `ReproductiveObservation` components above (three markers in all).
