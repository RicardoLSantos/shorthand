...
This narrative was authored as a companion to the FSH artifact [CDSHooksServiceDeclaration.fsh](https://github.com/RicardoLSantos/shorthand/blob/main/input/fsh/capabilities/CDSHooksServiceDeclaration.fsh) committed by T2 S20 (7 May 2026) and confirmed at IG v0.3.0 source HEAD `8333a04c4` (T2 S21, 12 May 2026). Drafted by T1 S34 (13 May 2026). The 4-service catalog (patient-view / order-sign / medication-prescribe / encounter-discharge) reflects the consensus pattern observed in CDS Hooks 2.0 reference implementations and the HL7 FHIR Intermediate Brasil 1ª Edição course (Mabi A. Souza + Lucas A. de Paula, 2026), Unit 4. The four-step validation framework citation (Wasylewicz & Scheepers-Hoeks, 2019) is Level 5 content-verified per T1 S32 graduation (DOI 10.1007/978-3-319-99713-1_11; CC BY 4.0 Open Access, 18 pp full read). Domain-specific thresholds (WHO PA, AASM sleep, RMSSD/SDNN) are operationalised in [LifestyleRiskAssessmentPlanDefinition](PlanDefinition-LifestyleRiskAssessmentPlanDefinition.html). The JSON Schema validation discussion is informed by U04-03 course exercise documenting the `sources` vs `source` plural-singular trap (Pitfall #95).

---

## ADDENDUM (T1 S47, 28 May 2026) — Integrated Walkthrough M4 cross-link + CQL Library discipline (Pitfall #112)

<!-- AUTHORED-BY-CLAUDE-T1-S47 - additive ADDENDUM per Lesson #431 frozen-at-birth -->

The four services declared above are now composed end-to-end in the new [Integrated Walkthrough (M4)](smart-cds-integrated-walkthrough.html) page, which traces a single clinical scenario from SMART App Launch → CDS Hooks discovery → `patient-view` hook → `lifestyle-risk-patient-view` service → `ClinicalImpressionAIAssessment` with CQL Library provenance + agentRecommendation extension → `CarePlanLifestyleMedicine`. The walkthrough demonstrates that the four-service catalog above is not a list of unrelated endpoints — it is a coordinated layer that, with the CDS-Hooks-2.0 prefetch and Card mechanics, composes into a working decision-support flow.

The CQL decision logic that the `lifestyle-risk-patient-view` service invokes is now also published as two thin `Library` resources ([`LibraryCVR003HRVRisk`](Library-LibraryCVR003HRVRisk.html) and [`LibraryMET002MetabolicOverride`](Library-LibraryMET002MetabolicOverride.html)) using the CQL `urn:cql:library:<id>:<version>` coordinate convention. Because that URN is intentionally outside the IG canonical namespace, both URNs are registered under `parameters: special-url:` in `sushi-config.yaml` — a discipline now formalised as **Pitfall #112** in the project ledger: any FHIR conformance resource with a non-namespace canonical (`urn:*`, external `http://*`) must be in `special-url`, or must use the IG canonical. Without that registration the IG Publisher emits a *URL Mismatch* error and the Card cannot link to its decision-rule provenance. The empirical fix and the refinement of the earlier Pitfall #61 caveat (which was about content-validation downstream of fragment CodeSystems, not URL-match) is recorded in [Implementation Scope and Roadmap §CQL Libraries](implementation-scope-and-roadmap.html) and in [LLM/AI Integration §CQL Library discipline](llm-ai-integration.html).
