...
## Known limitation in the IG's ConceptMaps (2026-09-23)

Four ConceptMaps use `openEHR-EHR-OBSERVATION.physical_activity_detailed.v0` as the system of one group: `ConceptMapFHIRToOpenEHR` (`group[1]`, target), `ConceptMapOpenEHRToFHIR` (`group[1]`, source), `ConceptMapOpenEHRToOMOP` (`group[1]`, source) and `ConceptMapVendorToOpenEHR` (`group[6]`, target), 19 mapped elements in all. That archetype is **not distributed** with this IG (see *Validation status* above), so these groups cannot be resolved from the IG. The IG Publisher reports this with three warning messages (the identifier cannot be resolved; the source and the target code system are not fully defined); they are suppressed, in Portuguese and in English, by six lines of `input/ignoreWarnings.txt`. The mappings are unchanged in this release. The 19 elements are step count, distance, active calories and activity minutes (moderate, vigorous or exercise). *Validation status* describes the archetype as a duplicate of the published CKM `physical_activity` archetype, while limitation 1 above notes that the CKM archetype models activity level and category only; how the two archetypes relate has not been checked, and whether these groups are re-targeted to a published archetype is undecided.
